...
public sealed class Actions : IAsyncDisposable, IDisposable {
	...
	[PublicAPI]
	public async Task<(bool Success, string Message)> Pause(bool permanent, ushort resumeInSeconds = 0, string? reason = null) {
	...
	public async Task<(bool Success, string Message)> Play(IReadOnlyCollection<uint> gameIDs, string? gameName = null) {
		...
		if (!Bot.CardsFarmer.Paused) {
			await Bot.CardsFarmer.Pause(true, "Manual play mode activated").ConfigureAwait(false);
		}
