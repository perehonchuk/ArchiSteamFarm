		...
		foreach (string botName in botNames) {
			switch (botName.ToUpperInvariant()) {
				...
				case "@PAUSED":
					...
					continue;
				case "@ENABLED":
					IEnumerable<Bot> enabledBots = Bots.Where(static bot => bot.Value.KeepRunning && bot.Value.BotConfig.Enabled).OrderBy(static bot => bot.Key, BotsComparer).Select(static bot => bot.Value);
